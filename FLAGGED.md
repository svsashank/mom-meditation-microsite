...
**F33 (adjacent, observed — not fixed) — summary / key-findings / AEO-snippet echo each other.** While auditing, I noticed the summary paragraph, the key-findings bullets, and the ≤100-word AEO snippet are all generated from the same abstract conclusion sentences, so the same sentence can appear in all three (and in the meta description). This is body-internal redundancy, distinct from the body→Limitations duplication that was the defined scope of this audit (now fixed). Left as-is to avoid re-touching the AEO snippet and register outside the requested pattern; can be de-duplicated in a follow-up if wanted (e.g., have key-findings carry only points not already in the summary).
